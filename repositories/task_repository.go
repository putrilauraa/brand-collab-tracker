...
func CreateTask(input TaskInput) (*models.Task, error) {
	...
	config.DB.Preload("Project").Preload("Project.Brand").Preload("Project.Brand.Category").First(&task, task.ID)
	return &task, nil
}

func GetTaskByProject(projectID uint) ([]models.Task, error) {
	var tasks []models.Task
	err := config.DB.Where("project_id = ?", projectID).Preload("Project").Preload("Project.Brand").Preload("Project.Brand.Category").Find(&tasks).Error
	return tasks, err
}

func GetTaskByID(id uint) (*models.Task, error) {
	var task models.Task
	err := config.DB.Preload("Project").Preload("Project.Brand").Preload("Project.Brand.Category").First(&task, id).Error
	return &task, err
}
